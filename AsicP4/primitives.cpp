...
#define CARD_RANK_LEAD_MASK 0x2000000000

class get_cardinality_rank : public ActionPrimitive<Data &, const Data &> {
  void operator ()(Data &rank, const Data &hash1) {
  	unsigned long long hash1L = hash1.get_uint64();
  	unsigned long long leadMaskL = CARD_RANK_LEAD_MASK;
  	unsigned int rankI = 1;
  	while (leadMaskL) {
  		if (hash1L & leadMaskL) {
  			rank.set(rankI);
  			break;
...
REGISTER_PRIMITIVE(get_cardinality_rank);

class update_cardinality_rank
  : public ActionPrimitive<RegisterArray &, const Data &, const Data &, Data &> {
  void operator ()(RegisterArray &buckets, const Data &index, const Data &newRank, Data &updateMean) {
  	unsigned int indexI = index.get_uint();
  	unsigned int newRankI = newRank.get_uint();
  	unsigned int oldRankI = buckets[indexI].get_uint();
  	if (oldRankI < newRankI) {
  		buckets[indexI].set(newRank);
  		updateMean.set(1);
  	}
  }
};

REGISTER_PRIMITIVE(update_cardinality_rank);

class update_cardinality_mean
  : public ActionPrimitive<const Data &, RegisterArray &, const RegisterArray &, const Data &, const Data &> {
  void operator ()(const Data &updateMean, RegisterArray &mean, const RegisterArray &buckets, const Data &bucketsNumber, const Data &useHarmonicMean) {
  	if (updateMean.get_uint()) {
  		unsigned long long newMean = 0;
  		double newSum = 0;
  		unsigned int rank = 0;
  		unsigned int bucketsNumberI = bucketsNumber.get_uint();
  		if (useHarmonicMean.get_uint()) {
			for (unsigned int i = 0; i < bucketsNumberI; i++) {
				rank = buckets[i].get_uint();
				newSum += (1.0 / (1 << rank));
			}
			newMean = bucketsNumberI * (bucketsNumberI / newSum);
  		}
  		else {
			for (unsigned int i = 0; i < bucketsNumberI; i++) {
				rank = buckets[i].get_uint();
			...
			newMean = bucketsNumberI * (1 << (unsigned int)(newSum / bucketsNumberI));
  		}
  		mean[0].set(newMean);
  	}
  }
...
REGISTER_PRIMITIVE(update_cardinality_mean);

class update_sample_y2u
  : public ActionPrimitive<const Data &, RegisterArray &, RegisterArray &, const Data &> {
  void operator ()(const Data &newHash1, RegisterArray &index, RegisterArray &data, const Data &dataSize) {
  	unsigned long long newHash1L = newHash1.get_uint64();
  	int dataSizeI = dataSize.get_int();
	int indexI = index[0].get_int();
  	int maxIndexI = indexI;
  	if (indexI == dataSizeI) {
  		maxIndexI -= 1;
  		unsigned long long maxHash1 = data[maxIndexI].get_uint64();
  		if (newHash1L >= maxHash1) {
  			return;
  		}
  	}
  	else {
  	...
  	unsigned long long middleHash1 = 0;	
  	int maxIndex = maxIndexI - 1;
  	int minIndex = 0;
  	int middleIndex = 0;
  	while (minIndex < maxIndex) {
  		middleIndex = (minIndex + maxIndex) / 2;
  		middleHash1 = data[middleIndex].get_uint64();
  		if (middleHash1 < newHash1L) {
  			minIndex = middleIndex + 1;
  		}
  		else if (middleHash1 > newHash1L) {
  	...
  	unsigned long long oldHash1 = data[startIndex].get_uint64();
  	if (oldHash1 < newHash1L) {
  		startIndex += 1;
  	}
  	for (int i = startIndex; i <= maxIndexI; i++) {
  		oldHash1 = data[i].get_uint64();
  		data[i].set(newHash1L);
  		newHash1L = oldHash1;
  	}
  }
};

REGISTER_PRIMITIVE(update_sample_y2u);
